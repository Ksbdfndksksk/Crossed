...
function onCreate()
 ...
 if not lowQuality then
    ...
    scaleObject('Tutorial_secret_bg', 1.6, 1.6);
...
function onUpdate()
    ...
    if (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 1280) and (getMouseY('camHUD') > 0 and getMouseY('camHUD') < 720 and mouseReleased('left')) or keyJustPressed('space') then

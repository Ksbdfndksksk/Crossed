...
function onCreate()
	...
	scaleObject('SansEvent_Dodge', 2.6, 2.6);		
	...
	scaleObject('SansEvent_Dodge_Hit', 2.6, 2.6);	
...
function onUpdate()
	if sansCanDoDodge == true and (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 128) and (getMouseY('camHUD') > 582.5 and getMouseY('camHUD') < 720 and mouseReleased('left')) then
